...
public class BidPreviewProductsPageTest extends MobileCoreBaseTest {
	...
	public BidPreviewProductsPageTest() {
	...
	public void after_NewAreaPricePageTest() {
		...
		if (bidSummaryPage.elementExistsByText("SIGN IN", language)) {
			// loginPage.doLogin(login.getUserName(), login.getPassword());
			loginPage.doLogin("proappqatest@gmail.com", "test123");
			homePage.clickAcceptTermsOfUse();
			homePage.clickNotificationsRemindMeLater();
		}
		language = loginPage.checkIfEnglishOrSpanish("SPECIAL OFFERS°OFERTAS ESPECIALES");
	...
	public void verifyProductIsDisplayedAndroid() throws Exception {
		bidSummaryPage.scrollToElementByText("Macropoxy 920 Pre-Prime", language);
		Assert.assertTrue(bidPreviewPage.elementExistsByTextLabel("Macropoxy 920 Pre-Prime", language));
	}

	@Test(groups = { "mobile_regression", "ios" }, description = "Verify product is displayed")
	public void verifyProductIsDisplayedIOS() throws Exception {
		bidSummaryPage.scrollToElementByText("Macropoxy 920 Pre-Prime", language);
		Assert.assertTrue(bidPreviewPage.elementExistsByTextLabel("Macropoxy 920 Pre-Prime", language));
	}
}
